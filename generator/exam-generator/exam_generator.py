...
class QuestionGenerator:
    ...
    def generate(self):
        ...
        for i in range(self.config.number_of_questions):
            questions.append(
                self.__generate_single_question(
                    self.config.questions_length[i],
    ...
    def __generate_single_question(
        self,
        ...
        question_text = TextGenerator.generate_question_text(
            random.randint(1, question_length)
        )
        ...
        for token in QuestionGenerator.__token_generator(
            self.config.answers_token, number_of_answers
        ):
            answer_text = TextGenerator.generate_text(random.randint(1, answer_length))
            answers.add_item(
                f"{token}{self.config.answers_separator.value}", answer_text
            )
            logger.debug(
    ...
    def __token_generator(answer_token: AnswerToken, number_of_answers: int):
        start_token = (
            answer_token.value if answer_token != AnswerToken.ROMAN_NUMBERS else 1
        )
