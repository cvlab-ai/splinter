...
def add_random_mark(image):
    ...
    def add_handwritten_mark(image, mark_type='doodle', alpha=random.uniform(0.03, 0.25), beta=random.uniform(0.03, 0.25), gamma=random.uniform(0.03, 0.25)):
        ...
        if mark_type == 'doodle':
            alpha = random.uniform(0.03, 0.5)
            beta = random.uniform(0.03, 0.5)
            gamma = random.uniform(0.03, 0.5)
            ...
            for _ in range(random.randint(10, 30)):
                ...
                gamma = random.uniform(0.01, 0.25)
                ...
                scale_factor = random.uniform(15, 40) / max(mark_width, mark_height)

                xvals = ((xvals - min(xvals)) * scale_factor + x + random.randint(-5, 5) - (mark_width * scale_factor) / 2).astype(int)
                yvals = ((yvals - min(yvals)) * scale_factor + y + random.randint(-5, 5) - (mark_height * scale_factor) / 2).astype(int)

                xvals_list.extend(xvals)
                yvals_list.extend(yvals)
            alpha = random.uniform(0.03, 0.5)
            beta = random.uniform(0.03, 0.5)
            gamma = random.uniform(0.03, 0.5)
            xvals, yvals = create_handwritten_doodle(alpha, beta, gamma)
            ...
            xvals_list.extend(xvals)
            yvals_list.extend(yvals)
            xvals, yvals = np.array(xvals_list), np.array(yvals_list)
        elif mark_type == 'tick':
        ...
        gradient = [
            f'#{max(0, min(255, int(chosen_color[0] * shade * random.uniform(0.9, 1.2) / 255))):02x}{max(0, min(255, int(chosen_color[1] * shade * random.uniform(0.9, 1.2) / 255))):02x}{max(0, min(255, int(chosen_color[2] * shade * random.uniform(0.9, 1.2) / 255))):02x}'
        ...
        for i in range(len(xvals) - 1):
            draw_blank.line((xvals[i], yvals[i], xvals[i + 1], yvals[i + 1]), fill=gradient[i], width=2)

        # Apply blur to the blank image
        blurred_image = blank_image.filter(ImageFilter.GaussianBlur(radius=random.randint(0, 1)))

        combined_image = Image.alpha_composite(image.convert("RGBA"), blurred_image)
    ...
    draw = ImageDraw.Draw(image)
    width, height = image.size
    mark_type = random.choice(['doodle', 'cross', 'tick', 'circle'])
