...
def group_by_size(contours: np.ndarray, similarity_prop: float = 1.1) -> List[List[int]]:
    ...
    while cont_idx < len(contours):
        if len(grouped[group_idx]) == 0 or cv2.contourArea(grouped[group_idx][0]) / cv2.contourArea(
                contours[cont_idx]) < similarity_prop:
...
def detect_lines(threshhold, kernel_size: Tuple[int, int]):
    ...
    for contour in contours:
        ...
        if kernel_size[0] > kernel_size[1]:  # Detect horizontal lines
            ...
            lines.append(np.array([[contour.min(axis=0)[0], y_mean], [contour.max(axis=0)[0], y_mean]]))
        else:  # Detect vertical lines
            x_mean = int(contour.mean(axis=0)[0])
            lines.append(np.array([[x_mean, contour.min(axis=0)[1]], [x_mean, contour.max(axis=0)[1]]]))
    return lines


def detect_boxes_middles(horizontal_lines: List[np.ndarray], vertical_lines: List[np.ndarray], shift: int = 10):
    def intersect(a, b):
    ...
    def find_neighbour_horizontal_line(_h_line: np.ndarray, _lines: List[np.ndarray], threshold: int = 5):
        for _line in _lines:
    ...
    for i, h_line_1 in enumerate(sorted_horizontal_lines[:-1]):
        h_line_2 = find_neighbour_horizontal_line(h_line_1, sorted_horizontal_lines[i+1:])
        if h_line_2 is None:
            continue
